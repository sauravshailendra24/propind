...
async def startup_event():
    ...
    asyncio.create_task(background_challenge_expiry_task())
...
async def background_challenge_expiry_task():
    """Expire daily challenge access after 24 hours. Payment starts a fresh challenge."""
    while True:
        try:
            async with common.db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute(
                        """UPDATE users_state
                           SET challenge_status='expired', challenge_active_until=NULL
                           WHERE challenge_active_until IS NOT NULL
                             AND challenge_active_until <= NOW()
                             AND challenge_status='active'"""
                    )
        except Exception as e:
            logger.error(f"[Challenge Expiry] {e}")
        await asyncio.sleep(30)
...
@app.get("/challenge-payment")
async def challenge_payment_page(request: Request):
    user = await get_current_user(request)
    if not user:
    ...
    if not state:
        return RedirectResponse(url="/")
    if not state.get('upstox_verified'):
        return RedirectResponse(url="/upstox-verify")
    status = await get_challenge_status(user['user_id'])
    if status.get('active'):
        return RedirectResponse(url="/dashboard")
    html_path = os.path.join(frontend_dir, "challenge_payment.html")
    if not os.path.exists(html_path):
        html_path = os.path.join(BASE_DIR, "challenge_payment.html")
    if not os.path.exists(html_path):
        return HTMLResponse("<h1>Payment page not found</h1>", status_code=500)
...
async def api_challenge_status(request: Request, order_id: str = None):
    ...
    if not user:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    result = await get_challenge_status(user['user_id'])
...
async def upstox_verify_page(request: Request):
    ...
    if not user:
        return RedirectResponse(url="/")
    state = await get_onboarding_state(user['user_id'])
    if state and state.get('upstox_verified'):
        return RedirectResponse(url="/challenge-payment")
...
async def get_dashboard(request: Request):
    ...
    if state and not state['upstox_verified']:
        return RedirectResponse(url="/upstox-verify")
    challenge_status = await get_challenge_status(user['user_id'])
    if not challenge_status.get('active'):
        return RedirectResponse(url="/challenge-payment")
    ...
    async with common.db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                "balance, daily_start_balance, challenge_active_until FROM users_state WHERE user_id=%s",
                (user_id,)
            )
            row = await cur.fetchone()
            if not row:
                return JSONResponse({"error": "State missing"}, status_code=400)
            upstox_ok, completed, dd_breached, bal, daily_start, challenge_until = row
            if not upstox_ok:
                return JSONResponse({"error": "Please open your Upstox account first."}, status_code=403)
            if not challenge_until:
                return JSONResponse({"error": "Your 4-day challenge has expired. Please pay ₹11 to start a fresh challenge."}, status_code=403)
            await cur.execute("SELECT %s > NOW()", (challenge_until,))
            if not bool((await cur.fetchone())[0]):
                return JSONResponse({"error": "Your 4-day challenge has expired. Please pay ₹11 to start a fresh challenge."}, status_code=403)
            if completed:
...
async def close_position(position_id: int, request: Request):
    ...
    async with common.db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                "SELECT assessment_completed, challenge_active_until FROM users_state WHERE user_id=%s",
                (user_id,)
            )
            row = await cur.fetchone()
            if row and (bool(row[0]) or not row[1]):
